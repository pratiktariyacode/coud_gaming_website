...
import Gameallvideo from './GameallVideo';

const  Gameall=()=> {
  return (
    <div>
              <h1 className="text-2xl new-game-title">Random all game</h1>
        {/* Horizontal scroll with scrollbar hidden */}
        <div className="grid-cols-4 grid justify-items-center all-game-card">
          {games.map((game) => (
            <div
              key={game.id}
            ...
            >
              <div className='Game-all-card-size'>
              <Gameallvideo
                image={game.img}
                video={game.video}
                title={game.title}
